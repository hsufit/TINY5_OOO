...
Rv32imReferenceCpu::Rv32imReferenceCpu(sc_core::sc_module_name name) : sc_core::sc_module(name) {
    SC_METHOD(tick);
    sensitive << clk.pos();
    dont_initialize();
}

void Rv32imReferenceCpu::reset_state() {
    registers_.fill(0);
    pc_ = 0;
...
void Rv32imReferenceCpu::raise_fault(unsigned code) {
    fault.write(true);
